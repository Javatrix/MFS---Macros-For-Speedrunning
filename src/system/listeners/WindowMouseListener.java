...
public class WindowMouseListener implements MouseListener {
    ...
    public void keyTyped(NativeKeyEvent e) {
        char c = e.getKeyChar();
        if (changing) {
            MacroManager.getMacros().get(index).setRunKey(c);
            changing = false;
            lastLabel.setText(MacroManager.getMacros().get(index).getName() + ": " + NativeKeyEvent.getKeyText(MacroManager.getMacros().get(index).getRunKey()));
        }
    }
